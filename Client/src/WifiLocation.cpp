...
String WifiLocation::MACtoString(uint8_t* macAddress) {
  ...
  snprintf(macStr, MAC_ADDR_SIZE, "%02X:%02X:%02X:%02X:%02X:%02X",
           macAddress[0], macAddress[1], macAddress[2], macAddress[3],
           macAddress[4], macAddress[5]);
  return String(macStr);
}

WifiLocation::WifiLocation(String googleKey) { _googleApiKey = googleKey; }

// Function to get a list of surrounding WiFi signals in JSON format to get
// location via Google Location API
...
// Calls Google Location API to get current location using surrounding WiFi
// signals inf
...
#else
  _client.setCACert(reinterpret_cast<const uint8_t*>(GlobalSignCA),
                    sizeof(GlobalSignCA));
#endif
#endif  // ESP8266
